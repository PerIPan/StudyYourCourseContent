...
export async function POST(request: NextRequest) {
  ...
  // Insert chunks one at a time with fully parameterized queries
  for (let i = 0; i < chunks.length; i++) {
    const chunk = chunks[i];
    const embeddingStr = `[${embeddings[i].join(',')}]`;
    await sql`
      INSERT INTO chunks (document_id, content, embedding_vec, page_number, chunk_index, course_slug)
      VALUES (${documentId}, ${chunk.content}, ${embeddingStr}::vector, ${chunk.pageNumber}, ${chunk.chunkIndex}, ${slug})
    `;
  }
